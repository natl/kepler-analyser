...
def fitTail(time, lumin, dlumin, params, method='power'):
  ...
  result = minimize(minFunc, params, args=(time, lumin, dlumin, f), method='leastsq') #TODO, LK: does not actually fit?
...
def burstFits(mt,ml,pt,mdl=None):
  ...
  except fit from 0.90Fpeak to 0.10Fpeak, rather than adaptively choose where to
  start fitting.
  '''
...
def smooth(t,f,r,tmin):
  ...
  assert len(t) == len(r) and len(t) == len(f), '''t,r,f should be the same shape'''
...
def separate(burstTime, burstLum, burstRad, modelID, outputDirectory):
  ...
  This program is designed to separate the bursts in the models into individual
  ...
  while jj < len(tim)-200:
    ...
    if (lum[jj] > minFlux) & (lum[jj]==max(nbhd)):
      ...
      if lum[jj]>10*persLumCompare and not isShock(tnbhd, nbhd, midIndex):
        ...
        convexities.append( convexity(burstRiseTimes, burstRiseLums, currentPeakLum, currentPersLum) )

        #Tail Fits
        pParams, pData, eParams, eData = burstFits(burstTim, burstLum, 0, mdl = None) #PeakTime is 0 at the moment
        ...
        for ii in np.arange(recordStartIndex,stopIndex):
          fluence += 0.5*((lum[ii] + lum[ii+1] - 2*currentPersLum)*(tim[ii+1] - tim[ii]))
...
class ModelAnalysis:
    ...
    def __init__(self, modelID, time, lum, radius):
        """
        Initialize analysis for given time, luminosity, and radius arrays.
        """
        ...
        self.clean()
        self.separated = {'num' : 0, 'endBurst' : False, 'burstID' : modelID} # Default result of self.separate()

    def clean(self):
        """
        Clean the light curve from duplicate times (time step smaller than numerical precision),
        negative and super-Eddington luminosities.
        """
        # Remove duplicate times and where lum <= 0; after removal we keep the first time value,
        # rather than averaging as the values only show varation at the 6th sig fig.
        ...
        if max_lum>1e39:
            logging.warn('%s exceeds Eddington, with peak lum at %e' % (self.modelID, max_lum))
            self.time, self.lum, self.radius = smooth(self.time, self.lum, self.radius, 0.125)
    ...
    def separate(self, output_dir):
        ...
        """
        x = separate(self.time, self.lum, self.radius, self.modelID, output_dir)
        self.separated = x
        return x
    ...
    def get_flag(self, twin_peaks=False, not_analysable=False):
        ...
        """
        # if not 'upTurn' in x: x['upTurn'] = False # TODO: remove if below works
        x = self.separated
        flag = int(0)
        if x['endBurst']: flag += 2**0                             # flag 01
        if self.superEddington: flag += 2**1                       # flag 02
        if 'upTurn' in x and x['upTurn']: flag += 2**2             # flag 04
        if twin_peaks: flag += 2**3                                # flag 08
        if x['num'] > 1:
            if (np.asarray(x['tdel'][1:])<100).any(): flag += 2**4 # flag 16
        if not_analysable: flag += 2**5                            # flag 32
    ...
    def get_alpha(self, accretion_lum):
        ...
        accretion_lum: accretion luminosity in erg/s (can be a numpy array with values for all bursts)
        """
        x = self.separated
        alphas = [(accretion_lum + ps)*dt/fl for (ps,dt,fl) in zip(x['persLum'], x['tdel'], x['fluen'])]
        x['alpha'] = alphas
        return alphas
    ...
    def get_mean_lcv(self, output_dir):
        """
        Calculate the mean burst light curve, and save it to 'mean.data' in output_dir
        """
        x = self.separated
        burstTime = self.time
        burstLum = self.lum
    ...
    def get_mean_values(self, key_table):
        ...
        if x['num'] > 0: # if there are bursts
            if x['num'] >= 3: # Ignore the first burst in train if there are more than 3 bursts
